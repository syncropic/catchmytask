/**
 * @file natural structured task language - Enhanced Version
  ...
  rules: {
    ...
    task_definition: ($) =>
      seq(
        ...
        "as",
        ":",
        repeat1(choice($.fetch_block, $.view_block))
      ),

    // Blocks
    fetch_block: ($) => seq("fetch", ":", repeat1($.fetch_operation)),
    view_block: ($) => seq("view", ":", repeat1($.view_operation)),
    ...
    dynamic_clause: ($) => seq("dynamic", ":", $.string_literal),
    input_clause: ($) => seq("input", ":", $.parameter_list),

    // Parameter list for input clause
    parameter_list: ($) => seq("(", optional($.parameter_items), ")"),
    parameter_items: ($) => seq($.parameter, repeat(seq(",", $.parameter))),
    parameter: ($) =>
      seq(
        field("key", $.identifier),
        "=",
        field("value", choice($.variable, $.string_literal, $.number))
      ),
