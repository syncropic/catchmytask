...
export const ActionInputForm: React.FC<DynamicFormProps> = ({
          ...
          mutate(
            {
              ...
              values: {
                ...
                // },
                profile: {
                  id: activeProfile?.id || identity?.email,
                  name: activeProfile?.name || identity?.email,
                },
              },
            },
            {
  ...
  const filterFields = (fields: Field[]): Field[] => {
    const fieldsToInclude = getFieldsToInclude(schema, action_mode);
    return fields.filter(
      (field) =>
        field.key &&
        include_items.includes(field.key) &&
  ...
  return (
    <>
      ...
      >
        ...
        {activeAgent || schema?.required.length === 1 ? (
