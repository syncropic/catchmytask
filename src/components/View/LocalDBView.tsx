...
export function LocalDBView({ view_record, view_item }: ViewProps) {
  // custom hooks
  const [fetchedSteps, setFetchedSteps] = useState<Record<string, boolean>>({});
  const { params } = useParsed();

  // const view_id = params?.view_id;
  const view_id = view_record?.id || params?.view_id;
  ...
  useEffect(() => {
    ...
    // );
    const allFetched = fetchedSteps[view_item?.id] === true;
  ...
  return (
    <div>
      ...
      <Box pos="relative">
        <LoadingOverlay
          visible={fetchedSteps[view_item?.id] !== true}
          zIndex={1000}
          overlayProps={{ radius: "sm", blur: 2 }}
        />
        {view_record && view_item && (
          <ExecutionDataFetcher
            ...
            onStepFetched={() => handleStepFetched(view_item?.id)}
          />
        )}
