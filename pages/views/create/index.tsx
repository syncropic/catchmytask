...
import { IField, IFilter, IDataModel, IIdentity } from "../interfaces";
...
export const PageCreate: React.FC<IResourceComponentsProps> = () => {
  ...
  const handleDataModelChange = (value: string[]) => {
    // Find the item in data_models where value matches the selected value
    const item = data_models.find((item) => item.value === value[0]);
    console.log("item", item);

    // Ensure that the item's fields and filters are arrays before setting them to state
    // If item?.fields or item?.filters is not an array, set to empty array as fallback
    setDataModelColumns(item?.fields || []);
    setDataModelFilters(item?.filters || []);

    // Set the selected data_model value in form state
  ...
  const filters_columns = useMemo<MRT_ColumnDef<IFilter>[]>(
    () => [
      ...
      },
      {
        ...
        header: "include",
        Cell: ({ row }) => {
          return <div>{JSON.stringify(row.original.include)}</div>;
        },
      },
      {
        accessorKey: "exclude",
        header: "exclude",
        Cell: ({ row }) => {
          return <div>{JSON.stringify(row.original.exclude)}</div>;
        },
      },
      {
