...
export const PageList: React.FC<IResourceComponentsProps> = () => {
  ...
  const action_options = actionOptionsData?.data
    ? actionOptionsData?.data
        ...
        .filter((option) =>
          option?.metadata?.resources?.includes("caesars_payments")
        )
    : [];
  ...
  const data_columns = useMemo<MRT_ColumnDef<IPayment>[]>(
    () => [
          ...
          <Anchor component={Text}>
            <Text
              ...
              onClick={() => {
                go({
                  to: {
                    resource: "caesars_payments", // resource name or identifier
                    action: "show",
      ...
      },
      {
        accessorKey: "pnr",
        header: "pnr",
        Cell: ({ row }) => <div>{row.original.pnr ?? ""}</div>,
      },
      ...
      // },
      {
        accessorKey: "payment_usage",
        header: "payment_usage",
        filterVariant: "multi-select",
        Cell: ({ row }) => <div>{row.original.payment_usage ?? ""}</div>,
      },
      {
        accessorKey: "payment_status",
        header: "payment_status",
      ...
      {
        accessorKey: "payment_author",
        header: "payment_author",
        filterVariant: "multi-select",
        Cell: ({ row }) => <div>{row.original.payment_author ?? ""}</div>,
      },
      {
