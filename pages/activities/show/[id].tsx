...
export const ShowPage: React.FC = () => {
  ...
  // // };
  // const profileState = {
  //   record: { id: params?.profileId },
  //   success_message_code: params?.profileId,
  //   read_record_mode: "remote",
  // };
  ...
  // } = useReadRecordByState(profileState);
  ...
  //     const newProfile = profileData?.data?.find(
  //       (item: any) =>
  //         item?.message?.code === params?.profileId || "read_record"
  //     )?.data[0];
  ...
  //   if (!appLoading && !sessionLoading && !profileLoading) {
  //     setLoading(false);
  //   }

  //   // Handle errors
  //   if (appError || sessionError || profileError) {
  //     setError(
  //       appError?.message ||
        ...
        <Panel defaultSize={30} minSize={0}>
          <div className="h-[85vh] flex flex-col">
            ...
            <div className="flex-none">
              {params?.id && <MessagesWrapper issue_id={params?.id} />}
            </div>
            {/* Bottom component */}
